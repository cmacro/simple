...
uchar *chr;
	...
	while( keylen ) {
	  ...
	  case SpanNode:
		if( keylen < len || memcmp (key, node->span->bytes, len) )
		  return NULL;
	  ...
	  case Array16:
		...
		if( chr = memchr (node->radix16->keys, *key++, len) ) {
...
{
ARTfan node1[1], node2[1], node3[1], node4[1];
ARTslot *prev, newslot[1], *lock, *slot;
...
uchar *chr, type;
	...
	while( off < keylen ) {
	  ...
	  node1->generic = (ARTgeneric*)(Arena + newslot->off * 8);
	  ...
	  case SpanNode:
		...
		for( idx = 0; idx < len; idx++ )
		  if( key[off] != node1->span->bytes[idx] )
			break;
		  else
			off++;
		...
		if( idx ) {
		  ...
		  memcpy (node2->span->bytes, node1->span->bytes, idx);
		  newslot->off = (uchar *)node2->span - Arena >> 3;
		  *node2->span->value = *node1->span->value;
		...
		if( off < keylen ) {
		  ...
		  if( !idx )
			*node3->radix4->value = *node1->span->value;
		  ...
		  node3->radix4->keys[0] = node1->span->bytes[idx++];
		...
		if( max - idx ) {
		  ...
		  memcpy (node4->span->bytes, node1->span->bytes + idx, max - idx);
		  *node4->span->next = *node1->span->next;
	  ...
	  case Array4:
		...
		for( idx = 0; idx < max; idx++ )
		  if( key[off] == node1->radix4->keys[idx] )
			break;

		if( idx < max ) {
		  slot = node1->radix4->radix + idx;
		  off++;
		  continue;
		}
		...
		if( max < 4 ) {
		  node1->radix4->keys[newslot->nslot] = key[off++];
		  slot = node1->radix4->radix + newslot->nslot++;
		  break;
		}
		...
		for( idx = 0; idx < max; idx++ ) {
		  node2->radix16->radix[idx] = node1->radix4->radix[idx];
		  node2->radix16->keys[idx] = node1->radix4->keys[idx];
		}

		node2->radix16->keys[max] = key[off++];
		*node2->radix16->value = *node1->radix4->value;
	  ...
	  case Array16:
		...
		if( chr = memchr (node1->radix16->keys, key[off], max) ) {
		  idx = chr - node1->radix16->keys;
		  slot = node1->radix16->radix + idx;
		  off++;
		  continue;
		}
		...
		if( max < 16 ) {
		  node1->radix16->keys[max] = key[off++];
		  slot = node1->radix16->radix + newslot->nslot++;
		  break;
		}
		...
		for( idx = 0; idx < max; idx++ ) {
		  slot = node1->radix16->radix + idx;
		  node2->radix48->radix[idx] = *slot;
		  node2->radix48->keys[node1->radix16->keys[idx]] = idx;
		}
		...
		*node2->radix48->value = *node1->radix16->value;
	  ...
	  case Array48:
		idx = node1->radix48->keys[key[off]];

		if( idx < 0xff ) {
		  slot = node1->radix48->radix + idx;
		  off++;
		  continue;
		}
		...
		if( newslot->nslot < 48 ) {
		  update48 = node1->radix48->keys + key[off++];
		  slot48 = newslot->nslot++;
		  slot = node1->radix48->radix + slot48;
		  break;
		}
		...
		for( idx = 0; idx < 256; idx++ )
		 if( node1->radix48->keys[idx] < 0xff ) {
		  slot = node1->radix48->radix + node1->radix48->keys[idx];
		  node2->radix256->radix[idx] = *slot;
		 }

		newslot->type = Array256;
		newslot->off = (uchar *)node2->radix256 - Arena >> 3;
		*node2->radix256->value = *node1->radix48->value;
	  ...
	  case Array256:
		slot = node1->radix256->radix + key[off++];
		continue;
	  ...
	  while( len = keylen - off ) {
		node1->span = (ARTspan *)(Arena + art_node(thread, SpanNode));

		if( oldvalue )
			*node1->span->value = *oldvalue;

		if( len > sizeof(node1->span->bytes) )
		  len = sizeof(node1->span->bytes);

		memcpy (node1->span->bytes, key + off, len);
		slot->off = (uchar *)node1->span - Arena >> 3;
		slot->type = SpanNode;
		slot->nslot = len;
		oldvalue = NULL;

		retvalue = slot = node1->span->next;
		off += len;
	  }
	...
	if( slot->off && slot->type > ValueSlot ) {
		node1->generic = (ARTgeneric *)(Arena + slot->off * 8);
		retvalue = node1->generic->value;
	} else
		retvalue = slot;
