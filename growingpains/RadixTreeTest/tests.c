...
#define START_TEST(name)    void fun##name()
#define tcase_add_test(name)    fun##name()

#define fail_unless(v)              \
    if (!v)  {                      \
        printf("Err %s %d \n", __FILE__, __LINE__); \
    }

#define fail_unlesss(expr, s, ...)    \
    if (!expr) {                     \
...
START_TEST(test_art_insert_search)
{
    ...
    while (fgets(buf, sizeof buf, f)) {
        ...
        fail_unlesss(line == val, "Line: %d Val: %u Str: %s\n", line,
            val, buf);
        line++;
    }
...
static int test_prefix_cb(void *data, const unsigned char *k, uint32_t k_len, void *val) {
    ...
    fail_unlesss(memcmp(k, p->expected[p->count], k_len) == 0,
        "Key: %s Expect: %s", k,
        p->expected[p->count]);
    p->count++;
    return 0;
...
START_TEST(test_art_iter_prefix)
{
    ...
    int idx = art_iter_prefix(&t, (unsigned char*)"api", 3, test_prefix_cb, &p);
    fail_unless(!idx);
    fail_unlesss(p.count == p.max_count, "Count: %d Max: %d", p.count, p.max_count);
    ...
    fail_unlesss(p4.count == p4.max_count, "Count: %d Max: %d", p4.count, p4.max_count);
    ...
    fail_unlesss(p5.count == p5.max_count, "Count: %d Max: %d", p5.count, p5.max_count);
...
int main (int argc, char** argv) {
    ...
    tcase_add_test(test_art_init_and_destroy);
    tcase_add_test(test_art_insert);
    tcase_add_test(test_art_insert_verylong);
    tcase_add_test(test_art_insert_search);
    tcase_add_test(test_art_insert_delete);
    tcase_add_test(test_art_insert_iter);
    tcase_add_test(test_art_iter_prefix);
    tcase_add_test(test_art_long_prefix);
    tcase_add_test(test_art_insert_search_uuid);
